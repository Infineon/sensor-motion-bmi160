...
#define BMI160_ERROR(x)     (CY_RSLT_CREATE(CY_RSLT_TYPE_ERROR, CY_RSLT_MODULE_BOARD_HARDWARE_BMI160, x))
...
cy_rslt_t mtb_bmi160_config_int(mtb_bmi160_t *obj, struct bmi160_int_settg *intsettings, cyhal_gpio_t pin, uint8_t intr_priority, cyhal_gpio_event_t event, cyhal_gpio_event_callback_t callback, void *callback_arg)
{
    ...
    {
        result = _mtb_bmi160_config_int(&(obj->intpin2), pin, false, intr_priority, event, callback, callback_arg);
    }
    else if (obj->intpin1 == NC)
    {
        result = _mtb_bmi160_config_int(&(obj->intpin1), pin, true, intr_priority, event, callback, callback_arg);
    }
    else if (obj->intpin2 == NC)
    {
        result = _mtb_bmi160_config_int(&(obj->intpin2), pin, true, intr_priority, event, callback, callback_arg);
    }
    else
    {
        result = CYHAL_BMI160_RSLT_ERR_ADDITIONAL_INT_PIN;
    }

    if (result == CY_RSLT_SUCCESS)
    {
        int8_t status = bmi160_set_int_config(intsettings, &(obj->sensor));
        if (status != BMI160_OK) result = BMI160_ERROR(status);
    }
